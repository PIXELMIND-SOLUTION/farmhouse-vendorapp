  ...
  Future<void> _handleLogin() async {
    ...
    if (success) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) =>  NavbarScreen(id: provider.vendor!.farmhouseId,)),
      );
    } else {
      _showErrorSnackBar(provider.errorMessage ?? 'Login failed');
      provider.clearError();
    }
  }
...
class _SignInButtonState extends State<_SignInButton>
  ...
  Widget build(BuildContext context) => GestureDetector(
        onTapDown: (_) {
          if (!widget.loading) _pressCtrl.forward();
        },
        onTapUp: (_) {
          _pressCtrl.reverse();
              ...
              decoration: BoxDecoration(
                ...
                gradient: LinearGradient(
                  colors: widget.loading
                      ...
                      : [_C.cyan, _C.blue, _C.violet],
                  stops: const [0.0, 0.5, 1.0],
                ),
                boxShadow: [
                  BoxShadow(
                    color: _C.cyan.withOpacity(widget.loading ? 0.1 : 0.3),
                    blurRadius: 20,
                    offset: const Offset(0, 8),
                  ),
                ],
